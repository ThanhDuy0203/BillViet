...
color_map= {15: (255, 0, 0), 16: (0, 255, 0), 17: (0, 0, 255), 18: (0, 255, 255)}


def get_list_file_in_folder(dir, ext= ['jpg', 'png', 'JPG', 'PNG']):
    included_extensions= ext 
    file_names= []
    all_file_names_and_dirs = os.listdir(dir)
    
    for fn in all_file_names_and_dirs:
...
def get_list_dir_file_in_dir_and_subdirs(folder, ext= ['jpg', 'png', 'JPG', 'PNG']):
    file_names= []
    ...
    for idx, box in enumerate(boxes):
        ...
        for coor in coors:
            if len(coor) < 8:
                ...
                if add_key_to_value and box['category_id'] in index.key():
...
class poly():
    ...
    def reduce_pts(self, dist_threshold= 7):
        last_pts= self.list_pts[0]
        filter_pts = []
        for i in range(1, len(self.list_pts)):
            cur_pts = self.list_pts[i]
            dist= euclidean_distance(last_pts, cur_pts)
            if dist > dist_threshold:
                filter_pts.append(last_pts)
                print('Keep point', i-1)
            last_pts = cur_pts
        
        if euclidean_distance(last_pts, self.list_pts[0]) > dist_threshold:
            filter_pts.append(last_pts)
    ...
    def check_max_wh_ratio(self):
        ...
        if len(self.list_pts) == 4:
            ...
            if (first_edge / second_edge) > 1:
                long_edge= (self.list_pts[0][0] - self.list_pts[1][0],
                            self.list_pts[0][1] - self.list_pts[1][1])
            else:
                long_edge= (self.list_pts[1][0] - self.list_pts[2][0],
                            self.list_pts[1][1] - self.list_pts[2][1])
        else:
            print('Check max wide height ratio. Polygon is not quadriltareal!')
    ...
    def check_horizontal_box(self):
        if len(self.list_pts) == 4:
            ...
            print('Check horizontal box. Polygon is not quadriltareal!')
    ...
    def get_horizontal_angle(self):
        ...
        if long_edge[0] == 0:
            ...
            angle_with_horizontal_line = math.atan2(long_edge[1], long_edge[0]) * 27.296
    ...
    def to_icdar_line(self, map_type= None):
        ...
        if len(self.list_pts) == 4:
            ...
            print('To icdar line. Polygon is not quadriltareal!')
        return line_str
...
def filter_data(src_dir, dst_dir, dst_anno= None):
